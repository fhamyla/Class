  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      <Card
        ...
        description="Enter student details below to add them to the roster."
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {error && (
            <div
              className="p-3 text-sm text-red-700 bg-red-50 rounded-lg border border-red-100"
              role="alert"
            >
              {error}
            </div>
        ...
        {students.length === 0 ? (
          ...
          <ul className="space-y-3" role="list">
            {students.map((student) => (
              <li
                key={student.id}
                className="flex items-center justify-between p-4 bg-white rounded-lg shadow-sm border border-warm-gray/20"
              >
