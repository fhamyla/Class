...
export function AttendanceMarker({
  ...
  return <div className="space-y-6 max-w-2xl mx-auto pb-20">
      <Card className="sticky top-0 z-10 shadow-md border-b-4 border-sage">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center space-x-2">
            <Calendar className="h-5 w-5 text-sage" />
            <input type="date" value={date} onChange={e => setDate(e.target.value)} className="block rounded-md border-warm-gray shadow-sm focus:border-sage focus:ring focus:ring-sage focus:ring-opacity-50 bg-white py-2 px-3" aria-label="Select date for attendance" />
          </div>
      ...
      <div className="space-y-3">
        ...
        return <div key={student.id} className={`
              ...
              <div className="flex items-center space-x-2">
                <button onClick={() => toggleStatus(student.id, 'present')} className={`
                  ...
                  `} aria-label={`Mark ${student.name} present`} aria-pressed={status === 'present'}>
                  <Check className="h-6 w-6" />
                </button>

                <button onClick={() => toggleStatus(student.id, 'absent')} className={`
                    flex items-center justify-center h-12 w-12 rounded-full border-2 transition-all
                  ...
                  `} aria-label={`Mark ${student.name} absent`} aria-pressed={status === 'absent'}>
                  <X className="h-6 w-6" />
                </button>
              </div>
            </div>;
      ...
      <div className="fixed bottom-6 left-0 right-0 px-4 flex justify-center pointer-events-none">
        <div className="pointer-events-auto shadow-xl rounded-full">
          <Button size="lg" onClick={handleSave} className={`min-w-[200px] rounded-full shadow-lg transition-all duration-300 ${isSaved ? 'bg-green-600' : ''}`}>
            {isSaved ? <>
