...
export function AttendanceMarker({
  ...
  return <div className="space-y-6 pb-20">
      <Card className="sticky top-0 z-10 shadow-md border-b-4 border-sage">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center space-x-2">
            ...
            <input type="date" value={date} onChange={e => setDate(e.target.value)} className="block rounded-md border-warm-gray shadow-sm focus:border-sage focus:ring focus:ring-sage focus:ring-opacity-50 bg-white py-2 px-3" />
          </div>
      ...
      {isLoading ? <div className="text-center py-8">Loading...</div> : <div className="space-y-3">
      ...
      <div className="fixed bottom-6 left-0 right-0 px-4 flex flex-col items-center pointer-events-none gap-2">
        {successMsg && <div className="bg-green-600 text-white px-4 py-2 rounded-full shadow-lg text-sm font-medium animate-in slide-in-from-bottom-5 fade-in">
            {successMsg}
          </div>}
        <div className="pointer-events-auto shadow-xl rounded-full">
          <Button size="lg" onClick={handleSave} disabled={isSaving} className="min-w-[200px] rounded-full shadow-lg">
