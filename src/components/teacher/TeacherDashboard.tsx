...
export function TeacherDashboard() {
  ...
  if (isLoading)
    return <div className="p-8 text-center">Loading class data...</div>;
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-wrap gap-1 bg-white p-1 rounded-lg border border-warm-gray/20 shadow-sm">
        <button
          ...
          className={`flex-1 py-2 px-4 text-xs sm:text-sm font-medium rounded-md transition-colors text-center ${activeTab === "overview" ? "bg-sage text-white shadow-sm" : "text-gray-600 hover:bg-gray-50"}`}
        >
          Overview
        </button>
        <button
          onClick={() => setActiveTab("attendance")}
          className={`flex-1 py-2 px-4 text-xs sm:text-sm font-medium rounded-md transition-colors text-center ${activeTab === "attendance" ? "bg-sage text-white shadow-sm" : "text-gray-600 hover:bg-gray-50"}`}
        >
          Mark Attendance
        </button>
        <button
          onClick={() => setActiveTab("students")}
          className={`flex-1 py-2 px-4 text-xs sm:text-sm font-medium rounded-md transition-colors text-center ${activeTab === "students" ? "bg-sage text-white shadow-sm" : "text-gray-600 hover:bg-gray-50"}`}
        >
          Manage Students
        </button>
      </div>
      ...
      {activeTab === "students" && (
        <StudentManagement students={students} onUpdate={loadData} />
      )}

      {activeTab === "attendance" && (
        <AttendanceMarker students={students} onSave={loadData} />
      )}
    </div>
  );
}
